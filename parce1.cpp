...
// void singleData(Server & server, std::string &line){
//     std::string value = "";
//     std::string key = "";
//     splitKeyValue(line, key, value, "listen", 0);
//     if (isDigit(key))
//         server.setListen(atoi(key.c_str()));
//     splitKeyValue(line, key, value, "host", 0);
//     if (!key.empty())
//         server.setHost(key);
//     splitKeyValue(line, key, value, "root", 0);
//     if (!key.empty())
//         server.setRoot(key);
//     splitKeyValue(line, key, value, "server_names", 0);
//     if (!key.empty())
//     {
...
//     //? Add logic to parse values the "http" block : error pages !!
